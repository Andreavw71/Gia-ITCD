...
/**
 * Servlet responsável por alterar informações do  Ajuda no banco de dados.
 * @author Elizabeth Barbosa Moreira/ Wendell Pereira de Farias
 * @version $Revision: 1.2 $
 */
public class AjudaAlterar extends AbstractAbacoServlet implements Flow, Form
{
	...
	private static final int REQUISICAO_ALTERAR_DESCRICAO_CAMPO_AJUDA = 6;
			  ...
			  LogException, PersistenciaException, AnotacaoException
	{
		switch (redirecionar(request))
		{
			...
			case REQUISICAO_ALTERAR_DESCRICAO_CAMPO_AJUDA:
				{
					alterarTelaCampoAjuda(request, response);
					break;
				}
	...
	protected int redirecionar(HttpServletRequest request)
	{
		...
		if (Validador.isStringValida(request.getParameter(PARAMETRO_CODIGO_AJUDA_ALTERAR)))
		{
			return REQUISICAO_ALTERAR_DESCRICAO_CAMPO_AJUDA;
		}
	 ...
	 * @implemented by Ricardo Vitor de Oliveira Moraes
	 */
	private TelaFuncionalidadeVo controladorInterativoBuffer(HttpServletRequest request)
	{
		TelaFuncionalidadeVo telaFuncionalidade = (TelaFuncionalidadeVo) getBuffer(request).getAttribute(TELA_FUNCIONALIDADE_VO);
		if(!Validador.isObjetoValido(telaFuncionalidade))
		{
	 ...
	 * @implemented by Ricardo Vitor de Oliveira Moraes
	 */
	private TelaFuncionalidadeVo controladorInterativoJSP(HttpServletRequest request)
	{
		...
		if (request.getParameter(CAMPO_SELECT_FUNCIONALIDADE_AJUDA) != null)
		{
			...
			if ((request.getParameter(CAMPO_CHECK_HABILITAR_AJUDA) != null) && 
				 request.getParameter(CAMPO_CHECK_HABILITAR_AJUDA).equals("on"))
			{
			  ...
			  ConsultaException, ConexaoException, PaginaNaoEncontradaException, TipoRedirecionamentoInvalidoException
	{
		...
		processFlow(VIEW_ALTERAR_AJUDA, request, response, FORWARD);
	}
	 ...
	 * @implemented by Ricardo Vitor de Oliveira Moraes
	 */
	private void listarDescricaoCampoAjuda(HttpServletRequest request, HttpServletResponse response) throws PaginaNaoEncontradaException, 
	 ...
	 * @implemented by Ricardo Vitor de Oliveira Moraes
	 */
	private void adicionarTelaCampoAjuda(HttpServletRequest request, HttpServletResponse response) throws PaginaNaoEncontradaException, 
			  TipoRedirecionamentoInvalidoException
	{
		TelaFuncionalidadeVo telaFuncionalidadeVo = controladorInterativoJSP(request);
		...
		if (Validador.isNumericoValido(codigo))
		{
			...
			telaCampoAjudaVoAux.setDescricaoAjudaCampo(TelaCampoAjudaVo.STRING_VAZIA);
			telaCampoAjudaVoAux.setStatusTelaCampo(new DomnStatusRegistro(DomnStatusRegistro.INATIVO));
			int status_confirmado = -1; //controla a funcionalidade confirmar
			int indice = ((List) telaFuncionalidadeVo.getTelaAjudaVo().getTelaCampoAjudaVo().getCollVO()).indexOf(telaCampoAjudaVoAtual);
			telaCampoAjudaVoAtual = (TelaCampoAjudaVo) ((List) telaFuncionalidadeVo.getTelaAjudaVo().getTelaCampoAjudaVo().getCollVO()).get(indice);
	...
	/**
	 * Método utilizado para altera os itens
	 * @param request
	 * @param response
	 * @throws PaginaNaoEncontradaException
	 * @throws TipoRedirecionamentoInvalidoException
	 ...
	 * @implemented by Ricardo Vitor de Oliveira Moraes
	 */
	private void alterarTelaCampoAjuda(HttpServletRequest request, HttpServletResponse response) throws PaginaNaoEncontradaException, 
			  TipoRedirecionamentoInvalidoException
	{
		TelaFuncionalidadeVo telaFuncionalidadeVo = controladorInterativoJSP(request);
		int codigo = StringUtil.toInt(request.getParameter(PARAMETRO_CODIGO_AJUDA_ALTERAR));
		if (Validador.isNumericoValido(codigo))
		{
			TelaCampoAjudaVo telaCampoAjudaVoAtual = new TelaCampoAjudaVo(codigo);
			int indice = ((List) telaFuncionalidadeVo.getTelaAjudaVo().getTelaCampoAjudaVo().getCollVO()).indexOf(telaCampoAjudaVoAtual);
			...
			telaFuncionalidadeVo.getTelaAjudaVo().getTelaCampoAjudaVo().setDescricaoAjudaCampo(telaCampoAjudaVoAtual.getDescricaoAjudaCampo());
			telaFuncionalidadeVo.getTelaAjudaVo().getTelaCampoAjudaVo().setCampoAjudaVo(telaCampoAjudaVoAtual.getCampoAjudaVo());
			telaFuncionalidadeVo.getTelaAjudaVo().getTelaCampoAjudaVo().setStatusTelaCampo(new DomnStatusRegistro(DomnStatusRegistro.INATIVO));
		}		
		getBuffer(request).setAttribute(TELA_FUNCIONALIDADE_VO, telaFuncionalidadeVo, request);
		processFlow(VIEW_ALTERAR_AJUDA, request, response, FORWARD);
	}
	...
	private void incluirTelaCampoAjuda(HttpServletRequest request, HttpServletResponse response) throws ObjetoObrigatorioException, 
			  LogException, ConexaoException, PersistenciaException, AnotacaoException, PaginaNaoEncontradaException, 
			  TipoRedirecionamentoInvalidoException
	{
		TelaFuncionalidadeVo telaFuncionalidadeVo = controladorInterativoJSP(request);
		Validador.validaObjeto(telaFuncionalidadeVo);
		...
		try
		{
			...
			int status_confirmar = -1; //controla os itens manipulados
			...
			for (Iterator iteTelaCampoAjudaVo = telaFuncionalidadeVo.getTelaAjudaVo().getTelaCampoAjudaVo().getCollVO().iterator(); iteTelaCampoAjudaVo.hasNext(); )
			{
				...
				if (telaCampoAjudaVoAtual.getStatusTelaCampo().getDomnValr() == status_confirmar)
				{
					telaCampoAjudaVoAtual.setStatusTelaCampo(new DomnStatusRegistro(DomnStatusRegistro.ATIVO));
					telaCampoAjudaVoAtual.setUsuarioLogado(getCodigoUsuarioLogado(request));
					telaCampoAjudaVoAtual.setConsultaCompleta(true);
					telaCampoAjudaVoAtual.setLogSefazVo(telaFuncionalidadeVo.getLogSefazVo());
					telaCampoAjudaBe.alterarTelaCampoAjuda(telaCampoAjudaVoAtual);
				}
			}
		}
