...
public class GIAITCDProtocolo extends AbstractAbacoServlet implements Form, Flow
{
           ...
           ConexaoException, ImpossivelCriptografarException, ValidacaoException, Exception
   {
      ...
      try
      {
         ...
         try
         {
            ...
            {
               //Retornar view de protocolo
               tipoProcessoVo = new TipoProcessoVo();
               tipoProcessoBe = new TipoProcessoBe();
               tipoProcessoVo = tipoProcessoBe.listarTipoProcesso(tipoProcessoVo);
               getBuffer(request).setAttribute(GIAITCD_VO, giaITCDVo, request);
               request.setAttribute(TIPO_PROCESSO_VO, tipoProcessoVo);
               processFlow(VIEW_PROTOCOLAR_GIA_ITCD, request, response, FORWARD);
            }
            else
            {
               ...
               //processFlow(VIEW_VALIDAR_PROTOCOLO_GIA_ITCD, request, response, FORWARD);
                throw new ValidacaoException("GIA-ITCD com Processo preenchido, favor utilizar a opção 4 - Validar Processo.");
            }
         }
      ...
      throws Exception
   {
      ...
      System.out.println("||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
      System.out.println("Parametro: " + params.toString());
      System.out.println("||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
