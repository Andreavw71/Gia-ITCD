...
/**
 * Servlet desenvolvida para reativar uma GIA
 * @author Anderson Boehler Iglesias Araujo
 * @version $Revision: 1.2 $
 */
public class GIAITCDReativar extends AbstractAbacoServlet implements Flow, Form
{
	private static final int REQUISICAO_SOLICITAR_REATIVAR_GIAITCD = 2;
	private static final int REQUISICAO_REATIVAR_GIAITCD = 3;
			  ...
			  DarException
	{
		switch (redirecionar(request))
		{
			...
			case REQUISICAO_SOLICITAR_REATIVAR_GIAITCD:
				{
					solicitarReativarGIAITCD(request, response);
					break;
				}
			case REQUISICAO_REATIVAR_GIAITCD:
				{
					reativarGIAITCD(request, response);
					break;
				}
		}
	}
	...
	protected int redirecionar(HttpServletRequest request)
	{
		...
		{
			return REQUISICAO_SOLICITAR_REATIVAR_GIAITCD;
		}
		else if (Validador.isStringValida(request.getParameter(BOTAO_CONFIRMAR)))
		{
			return REQUISICAO_REATIVAR_GIAITCD;
		}
	...
	private GIAITCDVo controladorInterativoJSP(HttpServletRequest request) throws ObjetoObrigatorioException
	{
		...
		if (getBuffer(request).getAttribute(GIAITCD_VO) == null)
		{
		...
		if (Validador.isStringValida(request.getParameter(CAMPO_JUSTIFICATIVA)))
		{
			giaITCDVo.getStatusVo().setJustificativaReativacao(request.getParameter(CAMPO_JUSTIFICATIVA));
		}
		return giaITCDVo;
	}
			  ...
			  TipoRedirecionamentoInvalidoException
	{
	   ...
	   giaITCDVo.setOrigem(DomnFuncionalidadeOrigem.REATIVAR_GIA_ITCD);
	   getBuffer(request).setAttribute(GIAITCD_VO, giaITCDVo, request);
	   request.setAttribute(PARAMETRO_CONSULTAR_REATIVAR, PARAMETRO_CONSULTAR_INATIVAR);   
		processFlow(FormAcesso.getUrlServletOriginalSemContexto(FormAcesso.CODIGO_REATIVAR_GIAITCD_PESQUISAR_GIAITCD, request), request, response, FORWARD);
	}
	 ...
	 * @implemented by Ricardo Vitor de Oliveira Moraes
	 */
	private void solicitarReativarGIAITCD(HttpServletRequest request, HttpServletResponse response) throws PaginaNaoEncontradaException, 
			  TipoRedirecionamentoInvalidoException, ObjetoObrigatorioException
	{
		GIAITCDVo giaITCDVo = controladorInterativoJSP(request);
		getBuffer(request).setAttribute(GIAITCD_VO, giaITCDVo, request);
		processFlow(VIEW_REATIVAR_GIAITCD, request, response, FORWARD);
	}
	 ...
	 * @implemented by Ricardo Vitor de Oliveira Moraes
	 */
	private void reativarGIAITCD(HttpServletRequest request, HttpServletResponse response) throws ObjetoObrigatorioException, 
			  ConsultaException, LogException, AnotacaoException, PersistenciaException, IntegracaoException, ConexaoException, 
			  DadoNecessarioInexistenteException, DarException, PaginaNaoEncontradaException, 
			  TipoRedirecionamentoInvalidoException
	{
		GIAITCDVo giaITCDVo = controladorInterativoJSP(request);
	   ...
	   String portClient = request.getHeader("CLIENTPORT");
		...
		try
		{
			...
			obterInformacoesLogSefaz(request, giaITCDVo);
			giaITCDBe.reativarGIAITCD(giaITCDVo, ipClient, portClient);
			getBuffer(request).setAttribute(GIAITCD_VO, giaITCDVo, request);
